...
interface TableRenderProps<T> {
  ...
  rows: T[] | null;
  dynamicColumns?: (rows: T[]) => Record<string, TableColumn<T>>;
...
function TableRender<T>({
  ...
  const StateEmpty = rows?.length === 0;
  ...
  const rowsData: T[] = Array.isArray(rows) ? (rows as T[]) : [];
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between w-full gap-2 group">
          ...
          {isSortable && (
            <div
              className={`flex items-center -space-x-1.5 transition-colors ${isSortedField ? "text-gray-200" : "text-[#D1D5DB] group-hover:text-gray-400"}`}
            >
              <svg
  ...
  const Td = ({
    ...
    const rendered = column?.component?.(data);

    // Render if component returns a React element OR a non-empty string
    if (
      rendered &&
      (React.isValidElement(rendered) || typeof rendered === "string")
    ) {
      return (
        <td
          ...
          <thead>
            <tr>
              {Object.keys(effectiveColumns).map((key) => (
                <Th key={key} field={key} column={effectiveColumns[key]} />
              ))}
            </tr>
          </thead>
          <tbody>
